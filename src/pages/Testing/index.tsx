...
const Testing: React.FC = () => {
  ...
  const [renderMode, setRenderMode] = useState<'dynamic_texture' | 'compute_shader' | 'shader_material' | 'procedural_texture'>('dynamic_texture')
...
fn main(input: VertexInput) -> VertexOutput {
  ...
  const generatePipelineFNodeMaterial = async () => {
    ...
    try {
      ...
      // Implement distinct Babylon.js approaches based on render mode
      let pipelineFMaterial = null
      
      switch (renderMode) {
        case 'dynamic_texture':
          // 🎨 CPU-Generated: DynamicTexture + StandardMaterial (most compatible)
          console.log('🎨 Using DYNAMIC_TEXTURE: CPU Pipeline F + StandardMaterial')
          pipelineFMaterial = createPipelineFTextureMaterial(scene, currentDP, dpCurveData, dpPaletteData, enhancedDP)
          break
          
        case 'compute_shader':
          // 🔥 WebGPU Compute: Generate texture using compute shader + StandardMaterial
          console.log('🔥 Using COMPUTE_SHADER: WebGPU compute + StandardMaterial')
        ...
        case 'shader_material':
          // ⚡ Real-time WGSL: ShaderMaterial with Pipeline F fragment calculation
          console.log('⚡ Using SHADER_MATERIAL: Real-time WGSL fragment shader')
        ...
        case 'procedural_texture':
          // 🌊 GPU-Generated: ProceduralTexture with WGSL + StandardMaterial
          console.log('🌊 Using PROCEDURAL_TEXTURE: GPU-generated texture + StandardMaterial')
          pipelineFMaterial = await createPipelineFProceduralMaterial(scene, currentDP, dpCurveData, dpPaletteData, enhancedDP)
          break
          
        default:
          console.warn('⚠️ Unknown render mode, falling back to DynamicTexture')
          pipelineFMaterial = createPipelineFTextureMaterial(scene, currentDP, dpCurveData, dpPaletteData, enhancedDP)
      }
      ...
      switch (renderMode) {
        case 'dynamic_texture':
          compiledShaders = {
            glsl: 'N/A (DynamicTexture uses CPU canvas generation)',
            wgsl: 'N/A (DynamicTexture uses CPU canvas generation)'
          }
          break
          
        case 'compute_shader':
          const computeShaderWGSL = generatePipelineFComputeShader(currentDP, dpCurveData, dpPaletteData)
          compiledShaders = {
            glsl: 'N/A (Compute shader uses WGSL only)',
            wgsl: computeShaderWGSL
          }
          break
          
        case 'shader_material':
          const fragmentShaderGLSL = generatePipelineFWithTextures(currentDP, targetAssignments)
          compiledShaders = {
            glsl: fragmentShaderGLSL,
            wgsl: 'WGSL version generated inline (see createPipelineFWGSLShaderMaterial)'
          }
          break
          
        case 'procedural_texture':
          compiledShaders = {
            glsl: 'N/A (ProceduralTexture implementation pending)',
            wgsl: 'N/A (ProceduralTexture implementation pending)'
          }
          break
      }
  ...
  const createDataTextures = (scene: any, curveData: any, paletteData: any) => {
    ...
    // Create palette data texture (back to simple RGBA approach for reliability)
    let paletteTexture = null
    if (paletteData && paletteData.length > 0) {
      // Use simple RGBA format that definitely works
      ...
      for (let i = 0; i < 256; i++) {
        ...
        paletteBytes[i * 4 + 3] = color.a !== undefined ? Math.floor(color.a * 255) : 255 // A
      }
      
      paletteTexture = new BABYLON.RawTexture(
        paletteBytes,
        256, 1, // 256x1 texture
        BABYLON.Engine.TEXTUREFORMAT_RGBA, // Simple RGBA format that works
        scene,
        false, false, // No mipmap, no invert
        BABYLON.Texture.NEAREST_SAMPLINGMODE // Exact pixel sampling
      )
      console.log(`🎨 Created palette data texture: 256x1 RGBA8 (${paletteData.length} colors)`)
    }
...
`
  ...
  // 🌊 GPU-GENERATED MODE: Create Pipeline F material using ProceduralTexture + StandardMaterial
  const createPipelineFProceduralMaterial = async (scene: any, selectedDP: any, curveData: any, paletteData: any, enhancedDP?: any) => {
    const { BABYLON } = babylonScene
    
    console.log('🌊 Creating Pipeline F ProceduralTexture (GPU-generated approach)...')
    
    // For now, fall back to DynamicTexture until ProceduralTexture is implemented
    const fallbackMaterial = createPipelineFTextureMaterial(scene, selectedDP, curveData, paletteData, enhancedDP)
    
    console.log('⚠️ Using DynamicTexture fallback for ProceduralTexture mode (implementation TODO)')
    return fallbackMaterial
  }
  ...
  // Generate Pipeline F compute shader (static approach)
  const generatePipelineFComputeShader = (selectedDP: any, curveData: any, paletteData: any): string => {
    // Bake curve data as WGSL array
    const curveArray = curveData && curveData.length > 0 
      ? curveData.slice(0, 256).map((v: number) => `${v}u`).join(', ')
      : Array.from({length: 256}, (_, i) => `${i}u`).join(', ')
    
    // Bake palette data as WGSL array
    const paletteArray = paletteData && paletteData.length > 0
...
@group(0) @binding(1) var<uniform> params: PipelineFParams;

struct PipelineFParams {
  texture_size: u32,
  world_scale: f32,
  curve_scaling: f32,
  curve_width: u32,
...
// Baked curve data (${curveData ? curveData.length : 256} values)
const CURVE_DATA = array<u32, 256>(${curveArray});

// Baked palette data (${paletteData ? paletteData.length : 256} colors)
const PALETTE_DATA = array<vec4f, 256>(${paletteArray});
...
fn main(@builtin(global_invocation_id) global_id: vec3<u32>) {
  ...
  if (x >= params.texture_size || y >= params.texture_size) {
    return;
  }
  
  // Convert to world coordinates
  let worldX = (f32(x) - f32(params.texture_size) * 0.5) * params.world_scale;
  let worldY = (f32(y) - f32(params.texture_size) * 0.5) * params.world_scale;
  
  // Pipeline F mathematics (baked for ${selectedDP.name})
  var p = vec2f(worldX, worldY);
  
  ${selectedDP['angular-distortion'] ? `
    // Baked angular distortion (freq=${selectedDP['angular-frequency']}, amp=${selectedDP['angular-amplitude']}, offset=${selectedDP['angular-offset']})
    let angle = atan2(p.y, p.x);
    let radius = length(p);
    let newAngle = angle + sin(angle * ${selectedDP['angular-frequency']}f + ${selectedDP['angular-offset']}f * 0.017453f) * ${selectedDP['angular-amplitude']}f * 0.01f;
    p = vec2f(cos(newAngle) * radius, sin(newAngle) * radius);
  ` : '// Angular distortion: DISABLED'}
  
  ${selectedDP['fractal-distortion'] ? `
    // Baked fractal distortion (strength=${selectedDP['fractal-strength']})
    p.x += sin(p.y * ${selectedDP['fractal-scale-1']}f) * ${selectedDP['fractal-strength']}f * 0.3f;
    p.y += cos(p.x * ${selectedDP['fractal-scale-2']}f) * ${selectedDP['fractal-strength']}f * 0.3f;
    p.x += sin(p.y * ${selectedDP['fractal-scale-3']}f) * ${selectedDP['fractal-strength']}f * 0.1f;
  ` : '// Fractal distortion: DISABLED'}
  
  ${selectedDP['distance-modulus'] > 0 ? `
    // Baked distance modulus (${selectedDP['distance-modulus']})
    let modulus = ${selectedDP['distance-modulus']}f;
    p = (p + modulus * 0.5f) % modulus - modulus * 0.5f;
  ` : '// Distance modulus: DISABLED'}
  
  // Baked distance calculation: ${selectedDP['distance-calculation']}
  var d = ${getBakedDistanceCalculation(selectedDP['distance-calculation'])};
  
  // Scale and index calculation
  let dPrime = d * params.curve_scaling;
  let curveIndex = u32(dPrime) % params.curve_width;
  let curveValue = CURVE_DATA[curveIndex];
  
  ${selectedDP['checkerboard-pattern'] && selectedDP['checkerboard-steps'] > 0 ? `
    // Baked checkerboard pattern (steps=${selectedDP['checkerboard-steps']})
    let checker = floor(d / ${selectedDP['checkerboard-steps']}f);
    if (checker % 2.0f > 0.5f) {
      curveValue = 255u - curveValue;
    }
  ` : '// Checkerboard: DISABLED'}
  
  // Direct palette lookup (no texture sampling)
  let finalColor = PALETTE_DATA[curveValue];
  
  // Write to output texture
  textureStore(output_texture, vec2u(x, y), finalColor);
}
`
  }

  // Get baked distance calculation formula
  const getBakedDistanceCalculation = (type: string): string => {
    switch(type) {
      case 'radial': return 'length(p)'
      case 'cartesian-x': return 'abs(p.x)'
      case 'cartesian-y': return 'abs(p.y)'
    ...
    <div className="testing-page">
      <div className="testing-content">
        <div className="testing-sidebar">
          ...
          <div className="form-group" style={{ marginTop: '20px', padding: '15px', border: '1px solid #ddd', borderRadius: '8px', backgroundColor: '#f9f9f9' }}>
            <label style={{ fontWeight: 'bold', marginBottom: '10px', display: 'block' }}>Pipeline F Render Mode:</label>
              ...
              <div style={{ marginTop: '15px', padding: '15px', border: '1px solid #ddd', borderRadius: '8px', backgroundColor: '#f8f9fa' }}>
                <h4 style={{ marginBottom: '15px', color: '#333' }}>
                  ...
                  <span style={{ fontSize: '14px', color: '#666', fontWeight: 'normal' }}>
                    ({renderMode === 'static' ? '🔥 Compute Shader' : '⚡ ShaderMaterial'})
                  </span>
                </h4>
                
                {renderMode === 'static' ? (
                  <div style={{ marginBottom: '20px' }}>
                    <h5>WebGPU Compute Shader (WGSL):</h5>
                    <div style={{ fontSize: '12px', color: '#666', marginBottom: '8px' }}>
                      Self-contained compute shader with baked Pipeline F mathematics and embedded data arrays
                    </div>
                    <pre style={{ 
                  ...
                  <>
                    <div style={{ marginBottom: '20px' }}>
                      <h5>Fragment Shader (GLSL):</h5>
                      <div style={{ fontSize: '12px', color: '#666', marginBottom: '8px' }}>
                        Real-time Pipeline F calculation with texture lookups
                      </div>
                      <pre style={{ 
                        backgroundColor: '#1e1e1e', 
                        color: '#d4d4d4', 
                    ...
                    <div style={{ marginBottom: '20px' }}>
                      <h5>WGSL (Auto-Generated by Babylon.js):</h5>
                      <div style={{ fontSize: '12px', color: '#666', marginBottom: '8px' }}>
                        WebGPU version automatically converted from GLSL above
                      </div>
                      <pre style={{ 
                        backgroundColor: '#1e1e1e', 
                        color: '#d4d4d4', 
